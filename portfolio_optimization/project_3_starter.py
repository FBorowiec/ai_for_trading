...
def generate_dollar_volume_weights(close, volume):
    ...
    for i in range(len(close)):
        tickers_sum.append(sum(close.values[i] * volume.values[i]))

    dollar_volume_weights = close * volume
    for i in range(len(close)):
...
def calculate_dividend_weights(dividends):
    ...
    return dividends.cumsum().div(dividends.cumsum().sum(axis=1), axis=0)
...
smart_beta_tracking_error = tracking_error(
    np.sum(index_weighted_returns, 1), np.sum(etf_weighted_returns, 1)
)
...
covariance_returns_correlation = np.linalg.inv(
    np.diag(np.sqrt(np.diag(covariance_returns)))
)
covariance_returns_correlation = pd.DataFrame(
    covariance_returns_correlation.dot(covariance_returns).dot(
...
project_helper.plot_covariance_returns_correlation(
    covariance_returns_correlation, "Covariance Returns Correlation Matrix"
)
...
raw_optimal_single_rebalance_etf_weights = get_optimal_weights(
    covariance_returns.values, index_weights.iloc[-1]
)
...
optim_etf_returns = generate_weighted_returns(
    returns, optimal_single_rebalance_etf_weights
)
optim_etf_cumulative_returns = calculate_cumulative_returns(optim_etf_returns)
project_helper.plot_benchmark_returns(
    index_weighted_cumulative_returns,
    optim_etf_cumulative_returns,
    "Optimized ETF vs Index",
)

optim_etf_tracking_error = tracking_error(
    np.sum(index_weighted_returns, 1), np.sum(optim_etf_returns, 1)
)
...
def rebalance_portfolio(returns, index_weights, shift_size, chunk_size):
    ...
    for i in range(chunk_size, len(returns), shift_size):
        new_returns = returns.iloc[i - chunk_size : i]
        new_cov = get_covariance_returns(new_returns)
        new_weights = get_optimal_weights(new_cov, index_weights.iloc[i - 1])
...
all_rebalance_weights = rebalance_portfolio(
    returns, index_weights, shift_size, chunk_size
)
...
def get_portfolio_turnover(
    all_rebalance_weights, shift_size, rebalance_count, n_trading_days_in_year=252
):
    """
    ...
    for i in range(1, len(all_rebalance_weights)):
        abs_reb = np.abs(all_rebalance_weights[i] - all_rebalance_weights[i - 1])
        portfolio_turnover += sum(abs_reb)

    yearly_rebalance_events = int(n_trading_days_in_year / shift_size)
    portfolio_turnover_yearly = (
        portfolio_turnover / rebalance_count
    ) * yearly_rebalance_events

    return portfolio_turnover_yearly
...
print(
    get_portfolio_turnover(
        all_rebalance_weights, shift_size, len(all_rebalance_weights) - 1
    )
)
